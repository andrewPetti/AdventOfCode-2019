...
long long Op4(std::vector<long long> &vect, long long pos, long long mode, long long relativeBase)
{
...
std::vector<long long> ExecuteProgram(std::vector<long long> &vect)//, long long id)
{
    //long long id = 0;
    ...
    while (cont)
    {
        ...
        else if (op == 3)
        {
            auto pos1 = *(it + 1);
            auto input = 0;
            if ( ballColumn - paddleColumn <0)
                input = -1;
            else if ( ballColumn - paddleColumn > 0)
                input = 1;
        ...
        if ( outputCounter == 3)
        {
            ...
            auto x = results[results.size()-3];
            auto y = results[results.size()-2];
            auto op = results[results.size()-1];

            if ( x == -1 && y == 0)
                std::cout << "Current Score: " << op <<std::endl;
...
void Process()
{
    ...
    auto input = inputs::GetInputs13();

    // auto results = ExecuteProgram(input);
    // auto count = 0;
    // for (int i=2; i<results.size(); i+=3)
    // {
    //     if ( results[i] == 2) //block
    //         count ++;
    // }

    // std::cout<< "The number of block tiles on the screen is: "<<count<<std::endl;

    input[0]= 2; //setting memory 0 to 2 to play for free
